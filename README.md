...
**停止維護原因：**
1. **雲端費用過高** — Cloud Run 需要 `min-instances=1` + Chromium 大記憶體，運行成本遠超廣告收入潛力
2. **UX 未完善** — 下載流程不夠直覺，仍需大幅打磨
3. **變現困難** — 以上兩點導致廣告變現模式不可行

**保留價值：** Threads 影片下載在開源社群中幾乎沒有替代方案（yt-dlp、cobalt 均不支援），此程式碼可作為未來參考。
